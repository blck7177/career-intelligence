...
def get_fit_report(fit_report_id: str, db: Session = Depends(get_db)):
...
def get_run_report(run_id: str, db: Session = Depends(get_db)):
    ...
    Looks up the run type, extracts report ID from task success events,
    and returns the corresponding report.
    """
    ...
    # Extract report ID from the task success event message.
    # Worker stamps the ID as "job_report_id=<id>" or "fit_report_id=<id>" in the event message.
    events = TaskEventRepository(db).list_for_run(run_id)
    report_id: Optional[str] = None
    for event in events:
        if event.event_type == "task_succeeded" and event.message:
