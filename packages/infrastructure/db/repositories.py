...
class StoryBankRepository:
    def __init__(self, session: Session) -> None:
        self._s = session

    def list_for_profile(self, profile_id: str) -> list[CandidateStoryBank]:
        return (
            self._s.query(CandidateStoryBank)
            .filter(CandidateStoryBank.profile_id == profile_id)
            .order_by(CandidateStoryBank.experience_ref, CandidateStoryBank.story_id)
            .all()
        )

    def get(self, story_bank_id: str) -> Optional[CandidateStoryBank]:
        return (
            self._s.query(CandidateStoryBank)
            .filter(CandidateStoryBank.id == story_bank_id)
            .first()
        )
    ...
    def upsert_stories(
        ...
        run_id: str,
        stories: list[dict],
    ) -> list[CandidateStoryBank]:
        """Replace all stories for this profile generated by a new agent run.

        Existing entries for the same profile are deleted first so that a
        re-run always produces a clean slate. User-edited entries (user_edited_at
        is set) are preserved — only AI-draft rows are replaced.
        """
        self._s.query(CandidateStoryBank).filter(
            CandidateStoryBank.profile_id == profile_id,
            CandidateStoryBank.user_edited_at.is_(None),
        ).delete(synchronize_session=False)

        rows = []
        for s in stories:
            row = CandidateStoryBank(
                profile_id=profile_id,
                workspace_id=workspace_id,
                run_id=run_id,
                experience_ref=s.get("experience_ref", ""),
                story_id=s.get("story_id", ""),
                workstream_title=s.get("workstream_title", ""),
                bullets_covered=s.get("bullets_covered"),
                narrative=s.get("narrative"),
                workflow=s.get("workflow"),
                evidence_items=s.get("evidence_items"),
                candidate_questions=s.get("candidate_questions"),
                do_not_claim=s.get("do_not_claim"),
                research_basis=s.get("research_basis"),
                user_edited_at=None,
            )
            self._s.add(row)
            rows.append(row)
    ...
    def update_story(
        ...
        *,
    ...
    ) -> Optional[CandidateStoryBank]:
        row = self.get(story_bank_id)
        if not row:
            return None
        ...
        if candidate_questions is not None:
            row.candidate_questions = candidate_questions
        if do_not_claim is not None:
            row.do_not_claim = do_not_claim
        if workstream_title is not None:
            row.workstream_title = workstream_title
