...
class CandidateStoryBank(Base):
    """One row per reconstructed workstream story for a candidate profile.

    Generated by agent.candidate_story_build; user edits are tracked via
    user_edited_at (null = AI draft, set = user has touched this entry).
    Resume tailor and future modules consume this table instead of doing
    ad-hoc story reconstruction from raw bullets.
    """

    __tablename__ = "candidate_story_bank"
    ...
    # Identifies which experience and workstream this story covers
    experience_ref: Mapped[str] = mapped_column(String(50), nullable=False)
    story_id: Mapped[str] = mapped_column(String(100), nullable=False)
    workstream_title: Mapped[str] = mapped_column(String(500), nullable=False)

    # Structured content — all written by the agent, editable by user
    bullets_covered: Mapped[Optional[list]] = mapped_column(JSON, nullable=True)
    narrative: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    workflow: Mapped[Optional[dict]] = mapped_column(JSON, nullable=True)
    ...
    do_not_claim: Mapped[Optional[list]] = mapped_column(JSON, nullable=True)
    research_basis: Mapped[Optional[list]] = mapped_column(JSON, nullable=True)

    user_edited_at: Mapped[Optional[datetime]] = mapped_column(
